...
//import covid_total_timeline from './total_timeline.js';
import notcovid from './notcoviddata.js';
...
class Notcovidchart extends Component {
    componentDidMount() {
        ...
        am4core.ready(function() {
            ...
            let lastDate = new Date(notcovid[notcovid.length - 1].date);
            ...
            function getSlideData(index) {
              if (index == undefined) {
                  index = notcovid.length - 1;
              }
          
              let data = notcovid[index];
  ...
  bubbleSeries.events.on("dataitemsvalidated", function() {
    ...
    bubbleSeries.dataItems.each((dataItem) => {
      ...
      //console.log(mapImage.dataItem)
      if (mapImage.dataItem.value == 0 || mapImage.dataItem.dataContext.disease == 'Z' ) {
        circle.hide(0);
      }
  ...
  slider.events.on("rangechanged", function(event) {
    let index = Math.round((notcovid.length - 1) * slider.start);
  ...
  lineChart.data = JSON.parse(JSON.stringify(notcovid));
 ...
 // var series = { active: activeSeries, confirmed: confirmedSeries, recovered: recoveredSeries, deaths: deathsSeries };
  ...
  let buttons = { active: activeButton };
  ...
  function changeDataType(name) {
    ...
    bubbleSeries.dataItems.each(function(dataItem) {
      ...
      dataItem.setValue('disease',dataItem.dataContext['disease'])
    })
  ...
  function setCountryData(countryIndex) {
    ...
    for (var i = 0; i < notcovid.length; i++) {
      ...
      if (countryData) {
        ...
        dataContext.active = countryData.length;
  ...
  function updateMapData(data) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      if (image) {
        ...
        image.dataItem.dataContext.active = 1;
        image.dataItem.dataContext.disease = di.disease
      }
        //console.log(image)
      ...
      if (polygon && image) {
        ...
        polygon.dataItem.dataContext.active = 1;
        image.dataItem.dataContext.disease = di.disease;
